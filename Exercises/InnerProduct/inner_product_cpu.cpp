 ...
 *  1. Allocate data on the CPU -> copy data to the device (how? Obviously I should not use 
 *      CUDA, HIP, ... as that would defeat the purpose)-> launch parallel execution. 
 *      The problem with this first approach is that if in a second moment I do not want to 
 *      use the GPU (I am on a cluster with no GPUs), then I need not copy the data to the device 
 *      and I need to execute the kernel on the host...
 *  2. I could use UVM, if there is support... But then I would not want to use explicit CUDA/HIP calls 
...
int main(int argc, char *argv[])
{
     ...
     */
    {
        ...
        */
        ...
        Kokkos::parallel_for(
            "y_init", Kokkos::RangePolicy<ExecSpace>(ExecSpace(), 0, N), KOKKOS_LAMBDA(const int64_t i) {
                y[i] = 1;
            });

        Kokkos::parallel_for(
            "x_init", Kokkos::RangePolicy<ExecSpace>(ExecSpace(), 0, N), KOKKOS_LAMBDA(const int64_t i) {
                x[i] = 1;
            });

        // the following loop should be optimised for CPU but poor for GPU (CHECK)
        Kokkos::parallel_for(
            "A_init", Kokkos::RangePolicy<ExecSpace>(ExecSpace(), 0, N), KOKKOS_LAMBDA(const int64_t i) {
                for (int j = 0; j < N; ++j)
                {
                    A[i * N + j] = 1;
                }
